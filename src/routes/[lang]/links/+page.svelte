  ...
  <section class="py-16 bg-gradient-to-br from-primary/10 via-background to-primary/5">
    <div class="container text-center">
      ...
      <p class="text-lg text-muted-foreground">
        {@html md(translate(linksContent.subtitle, lang))}
      </p>
    </div>
  </section>
  ...
  <section class="py-16 bg-muted/30">
    <div class="container">
      <h2 class="text-2xl font-bold text-center mb-8">
        {@html md(translate(linksContent.partnersTitle, lang))}
      </h2>
      <div class="flex flex-wrap justify-center items-center gap-8">
        {#if linksContent.partners}
          {#each linksContent.partners as partner}
            <a
              href={partner.url}
              target="_blank"
              rel="noopener noreferrer"
              aria-label={translate(partner.alt, lang) || partner.name}
            >
              <img
                src={partner.logo}
                alt={translate(partner.alt, lang) || partner.name}
                class="h-16 rounded hover:opacity-80 transition-opacity"
              />
            </a>
          {/each}
        {/if}
